...
export async function POST(req: NextRequest) {
  ...
  // Fetch real Google suggestions in parallel
  const seeds = [
    keyword,
    `${keyword} tips`,
    `best ${keyword}`,
  ...
  const arrays = await Promise.all(seeds.map(q => fetchSuggestions(q)));
  const suggestions = dedup(arrays.flat()).filter(s => s !== keyword).slice(0, 35);

  const suggestionBlock = suggestions.length > 0
    ? `Real Google Autocomplete suggestions for this keyword right now:\n${suggestions.map(s => `- ${s}`).join("\n")}`
    : "No autocomplete data available — use your general knowledge.";
...
Based on these real search patterns, return a concise analysis as JSON only (no markdown):
{
  "summary": "2-3 sentences on what's driving current interest in this keyword",
  "rising_trends": ["5-7 specific rising sub-topics or angles people are searching"],
  "audience_intent": "1-2 sentences on who is searching this and what they want",
  "content_opportunities": ["3-4 specific content ideas with clear angles"],
