...
function QuickWinCard({ win, index, researchData, bizName, personas, onAgentOutput }: WinCardProps) {
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden">
      ...
      >
        <span className="flex-shrink-0 w-6 h-6 rounded-full bg-[#eff6ff] text-[#275fe8] text-xs font-extrabold flex items-center justify-center mt-0.5">
          {index + 1}
        </span>
      ...
      {open && (
        <div className="border-t border-slate-100 px-4 py-4 bg-slate-50">
          ...
          {personas.length > 0 && (
            <div className="mb-4">
              <p className="text-xs font-extrabold text-[#275fe8] uppercase tracking-wide mb-2">Who are we targeting?</p>
          ...
          {steps.length > 0 && (
            <div className="mb-4">
              <p className="text-xs font-extrabold text-[#275fe8] uppercase tracking-wide mb-2">How to Complete</p>
              <ol className="space-y-2">
                {steps.map((s, si) => (
                  <li key={si} className="flex gap-2.5 text-sm text-slate-700">
                    <span className="flex-shrink-0 w-5 h-5 rounded-full bg-white border border-[#bfdbfe] text-[#275fe8] text-xs font-bold flex items-center justify-center mt-0.5">
                      {si + 1}
                    </span>
          ...
          {!output && !streamText && (
            <button
              onClick={activate}
              disabled={running}
              className="w-full bg-gradient-to-r from-[#275fe8] to-[#1a4fd0] text-white font-bold rounded-xl py-2.5 text-sm flex items-center justify-center gap-2 disabled:opacity-60 hover:shadow-md transition"
            >
              {running ? (
                <>
          ...
          {(streamText || output) && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-extrabold text-[#275fe8] uppercase tracking-wide">
                  {agentMeta?.label} Agent Output{output ? ` · ${output.timestamp}` : ""}
                  {running && <span className="inline-block w-1.5 h-3.5 bg-[#275fe8] ml-1 animate-pulse align-middle" />}
                </p>
  ...
  return (
    <div className="view-enter">
      ...
      <Section title="Overview — Business, Gaps & Strengths" defaultOpen>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            {services.length > 0 && (
              <>
                <p className="text-sm font-extrabold text-[#275fe8] uppercase tracking-wide mb-1">Services</p>
            ...
            {strengths.length > 0 && (
              <>
                <p className="text-sm font-extrabold text-[#275fe8] uppercase tracking-wide mb-1">Strengths</p>
          ...
          <div>
            {gaps.length > 0 && (
              <>
                <p className="text-sm font-extrabold text-[#275fe8] uppercase tracking-wide mb-1">Marketing Gaps</p>
                <ul className="list-disc list-inside text-sm text-slate-700 space-y-1">
      ...
      <Section title="SEO & Keywords">
        {onExploreTrends && industry && (
          <div className="flex justify-end mb-3">
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-bold text-[#275fe8] bg-[#eff6ff] border border-[#bfdbfe] px-3 py-1.5 rounded-full hover:bg-[#ede5f6] transition"
            >
        ...
        {keywords.length > 0 && (
          <div className="mb-4">
            <p className="text-sm font-extrabold text-[#275fe8] uppercase tracking-wide mb-3">Long-tail Keyword Opportunities</p>
        ...
        {Object.keys(paa).length > 0 && (
          <div className="mt-4">
            <p className="text-sm font-extrabold text-[#275fe8] uppercase tracking-wide mb-3">People Also Ask</p>
            <div className="grid md:grid-cols-2 gap-2">
        ...
        {techIssues.length > 0 && (
          <div className="mt-4">
            <p className="text-sm font-extrabold text-[#275fe8] uppercase tracking-wide mb-2">Technical Issues to Fix</p>
            <ul className="list-disc list-inside text-sm text-slate-700 space-y-1">
        ...
        {personas.length > 0 ? (
          <div className="flex flex-col gap-4">
            {personas.slice(0, 4).map((p, i) => (
              <div key={i}>
                <p className="font-extrabold text-[#275fe8] text-base">{p.persona_name ?? `Segment ${i + 1}`}</p>
      ...
      <Section title="Download Report">
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 text-center bg-[#275fe8] hover:bg-[#1a4fd0] text-white text-sm font-semibold py-2.5 rounded-xl transition"
          >
            Export PDF
          </button>
          <button
